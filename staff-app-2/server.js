...
if (MONGODB_URI) {
  ...
  client.connect()
    .then(() => {
      mongoCol = client.db('gikoudb').collection('staff_records');
      staffNamesCol = client.db('gikoudb').collection('staff_names');
      staffSettingsCol = client.db('gikoudb').collection('staff_settings');
      staffAccountsCol = client.db('gikoudb').collection('staff_accounts');
      actionItemsCol = client.db('gikoudb').collection('action_items');
...
<title>🦷 歯科在庫管理（技工）ダッシュボード | 歯科在庫管理（技工）</title>
...
<div class="cert">
  ...
  <div class="footer">
    ...
    <div class="clinic">歯科在庫管理（技工）</div>
  </div>
</div>
<script>
function updateBody() {
