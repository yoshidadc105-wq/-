...
app.post('/api/ai-feedback/:name', async (req, res) => {
  ...
  const prompt = `以下は歯科クリニックスタッフ「${name}」さんの評価データです。このデータをもとに、院長が本人に渡す総合フィードバック文章を日本語で書いてください。

【歯科クリニックの仕事の特性（フィードバックに必ず活かすこと）】
- 患者さんは緊張・不安を抱えて来院するため、スタッフの笑顔・声のトーン・言葉選びが患者体験に直結する
- チームワークが治療の質と安全に直接影響する（アシスタント、受付、歯科衛生士、ドクターの連携）
- 「また来たい」と思ってもらえる医院づくりはスタッフ全員の日々の行動が土台になる
- 報告・連絡・相談の徹底が医療ミス防止と院内の信頼関係の構築につながる
- 自己研鑽（技術・知識の向上）が患者さんへの説明の質や処置サポートレベルに反映される
- スタッフ自身が前向きに働ける雰囲気が、患者さんへの対応にもそのまま波及する

【執筆ルール】
- 冒頭の挨拶・書き出し（「〇〇さんへ」「院長より」等）は不要。すぐ本文を始める
- 各セクションは見出し【 】付きで書く
- 評価内容を歯科クリニックの現場に結びつけて具体的に書く（例：「患者さんへの声かけ」「治療中のアシスト」「チームへの情報共有」「診療後のフォロー」など）
- 自己評価と周囲評価にずれがある項目は「〇〇については自分では△△と感じているようですが、周囲からは□□と見られています。歯科の現場では〜という場面でその差が出やすく〜」のように具体的に書く
- 自由記述コメントが含まれる場合は内容を引用・言い換えて文章に組み込む
- 各セクション3〜5文、読んで「自分のことだ」と感じられる具体性を持たせる
- 来期の目標は「毎朝〇〇をする」「〇〇の場面で〇〇を意識する」レベルの行動目標にする

【セクション構成】
【総合所感】
【強みと評価できる点】
【自己認識と周囲の評価のずれ】（ずれがある場合のみ記載）
【歯科スタッフとして伸ばしてほしい点】
【来期の具体的な行動目標】
